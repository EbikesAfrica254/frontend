...
export async function authenticatedWorkforceFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  expectedClientId?: string,
): Promise<T> {
  const token = await getAuthToken(expectedClientId);
  return baseWorkforceFetch<T>(endpoint, options, {
    headers: {
...
function baseWorkforceFetch<T>(
  ...
  return baseFetch<T>(
    `${process.env.WORKFORCE_SERVICE_BASE_URL}`,
    endpoint,
    mergedOptions,
    configuration,
  );
