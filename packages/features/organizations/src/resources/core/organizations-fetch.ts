...
export async function authenticatedOrganizationsFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  expectedClientId?: string,
): Promise<T> {
  const token = await getAuthToken(expectedClientId);
  return baseOrganizationsFetch<T>(endpoint, options, {
    headers: {
...
function baseOrganizationsFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  ...
  return baseFetch<T>(
    `${process.env.ORGANIZATIONS_SERVICE_BASE_URL}`,
    endpoint,
    mergedOptions,
    configuration,
  );
