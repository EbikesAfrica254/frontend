...
export async function authenticatedNotificationsFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  expectedClientId?: string,
): Promise<T> {
  const token = await getAuthToken(expectedClientId);
  return baseNotificationsFetch<T>(endpoint, options, {
    headers: {
...
function baseNotificationsFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  ...
  return baseFetch<T>(
    getRequiredEnv("NOTIFICATIONS_SERVICE_BASE_URL"),
    endpoint,
    mergedOptions,
    configuration,
  );
...
async function getAuthToken(expectedClientId?: string): Promise<string> {
  ...
  if (expectedClientId && session.aud) {
    if (!validateAudience(session.aud, expectedClientId)) {
      throw new Error(
        `Invalid token audience: Expected ${expectedClientId}, got ${
          typeof session.aud === "string" ? session.aud : session.aud.join(", ")
        }`,
      );
    }
  }
