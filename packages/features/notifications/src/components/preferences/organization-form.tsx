...
import {
  CreateOrganizationPreferenceFormData,
  createOrganizationPreferenceSchema,
} from "../../schemas/preference-schemas";
import { NotificationCategory, NotificationChannel } from "../../types/enums";

interface OrganizationPreferenceFormProps {
  onSubmit: (
    data: CreateOrganizationPreferenceFormData,
  ) => void | Promise<void>;
}

export function OrganizationPreferenceForm({
  onSubmit,
}: OrganizationPreferenceFormProps) {
  const {
    control,
    formState: { errors, isSubmitting },
    handleSubmit,
    register,
  } = useForm<CreateOrganizationPreferenceFormData>({
    defaultValues: {
      category: undefined,
      channel: undefined,
      enabled: true,
    },
    resolver: zodResolver(createOrganizationPreferenceSchema),
  });

  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="space-y-4">
        <h3 className="text-lg font-medium">Channel Gate</h3>
