...
import { organizationPreferenceParamsParser } from "../../lib/organization-preferences-params-parser";
import { NotificationCategory, NotificationChannel } from "../../types/enums";
import { FilterSelect } from "@repo/ui/tables/filter-select";

interface OrganizationPreferenceFiltersProps {
  onFilterChange?: () => void;
}

export function OrganizationPreferenceFilters({
  onFilterChange,
}: OrganizationPreferenceFiltersProps) {
  const [isPending, startTransition] = useTransition();

  const [filters, setFilters] = useQueryStates(
    {
      category: organizationPreferenceParamsParser.category,
      channel: organizationPreferenceParamsParser.channel,
      enabled: organizationPreferenceParamsParser.enabled,
    },
    {
      shallow: false,
      startTransition,
