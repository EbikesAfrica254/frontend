...
export async function authenticatedMakerCheckerFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  expectedClientId?: string,
): Promise<T> {
  const token = await getAuthToken(expectedClientId);
  return baseMakerCheckerFetch<T>(endpoint, options, {
    headers: {
...
function baseMakerCheckerFetch<T>(
  endpoint: string,
  options: RequestInit = {},
  ...
  return baseFetch<T>(
    `${process.env.MAKER_CHECKER_SERVICE_BASE_URL}`,
    endpoint,
    mergedOptions,
    configuration,
  );
